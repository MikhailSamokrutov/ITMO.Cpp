...
public:
	...
	Time(int h, int m, int s) : hours(h), minutes(m), seconds(s) {
		if (h > 23)
			throw InvalidTimeEx("Ошибка ввода. Введите не более 23:59:59");
		if (m > 59 || s > 59)
			throw InvalidTimeMSEx("Ошибка ввода. Минуты и секунды не могут превышать 59");
		timeAdjust(h, m, s);
	}
	Time(double num) {
...
void Time::setTime() {
	try
	{
		...
		do {
			...
			cin >> h >> m >> s;
			if (h > 23)
				cout<<"Ошибка ввода. Введите не более 23:59:59\n";
			if (m > 59 || s > 59)
				cout<<"Ошибка ввода. Минуты и секунды не могут превышать 59\n";
